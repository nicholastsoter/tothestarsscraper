...
class PostgresSearchStore:
    """Same interface as SearchStore, backed by Postgres instead of a local
    SQLite file — used in production on Vercel, where the function
    filesystem doesn't persist data between requests or cold starts. This
    table is created fresh via the Vercel Postgres integration, so unlike
    SearchStore there's no pre-existing-database migration to worry about."""
    ...
    def count_recent_searches(self, username: str, since_epoch: float) -> int:
        ...
        try:
            row = conn.execute(
                """
                SELECT COUNT(*) AS n FROM searches
                WHERE username = %s AND timestamp >= %s AND was_live_call = 1
                """,
                (username, since_epoch),
            ).fetchone()
            return row["n"]
    ...
    def get_search(self, search_id: int) -> Optional[dict]:
        conn = self._connect()
        try:
            row = conn.execute("SELECT * FROM searches WHERE id = %s", (search_id,)).fetchone()
            if row is None:
                return None
            summary = _row_to_summary(row)
            summary["results"] = json.loads(row["results_json"])
    ...
    def set_contacted(self, place_id: str, contacted: bool, username: str, details: Optional[dict] = None) -> dict:
        ...
        try:
            ...
            conn.execute(
                f"""
                INSERT INTO contacted (place_id, contacted, contacted_at, username, {", ".join(columns)})
                VALUES (%s, %s, %s, %s, {", ".join("%s" for _ in columns)})
                ON CONFLICT (place_id) DO UPDATE SET
                    contacted = EXCLUDED.contacted,
                    contacted_at = EXCLUDED.contacted_at,
                    username = EXCLUDED.username,
                    {", ".join(f"{c} = COALESCE(EXCLUDED.{c}, contacted.{c})" for c in columns)}
                """,
                (place_id, 1 if contacted else 0, contacted_at, username, *(details.get(c) for c in columns)),
            )
            conn.commit()
    ...
    def count_contacted_since(self, since_epoch: float) -> int:
        ...
        try:
            row = conn.execute(
                "SELECT COUNT(*) AS n FROM contacted WHERE contacted = 1 AND contacted_at >= %s",
                (since_epoch,),
            ).fetchone()
            return row["n"]
        finally:
